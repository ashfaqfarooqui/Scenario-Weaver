...
| `num_scenarios` | integer | yes | n/a | How many distinct scenarios to generate. Must be at least 1. Values above 1 stratify every declared `position:`/`speed:` range (any actor, ego included) into that many strata and assign one scenario per stratum, seeded by the CLI's `--seed`; a blocking clause still rules out exact duplicates underneath. A spec with no ranges – every value fixed – produces one scenario regardless of this field. See [architecture.md](architecture.md#constraint-modes-and-multi-scenario-diversity). |
