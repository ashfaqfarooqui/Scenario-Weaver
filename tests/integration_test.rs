...
#[test]
fn test_frenet_lateral_position_bounds() {
    // Test that vehicles stay within road boundaries in Frenet scenarios
    let yaml_content = r#"
scenario_type: cut_in_left
time_step: 0.5
duration: 8.0
...
"#;
    ...
    let scenario = scenario_generator::generate_single_scenario(yaml_content)
        .expect("Should generate Frenet scenario successfully");

    let road_width = scenario.road.num_lanes as f64 * scenario.road.lane_width;
    assert_eq!(road_width, 10.5, "Road should be 3 lanes × 3.5m = 10.5m wide");

    // Verify all actors stay within road boundaries
    for actor in &scenario.actors {
        for (_time_idx, state) in actor.states.iter().enumerate() {
            if let Some(frenet) = &state.frenet {
                assert!(
                    frenet.t >= 0.0,
                    "Actor {} at t={}s has lateral position {} < 0 (left edge violation)",
                    actor.id,
                    state.time,
                    frenet.t
                );
                assert!(
                    frenet.t <= road_width,
                    "Actor {} at t={}s has lateral position {} > {} (right edge violation)",
                    actor.id,
                    state.time,
                    frenet.t,
                    road_width
    ...
    println!("Frenet lateral position bounds test passed");
    println!("  Road width: {}m", road_width);
    println!("  All actors stayed within [0, {}]m bounds", road_width);
}

#[test]
fn test_frenet_initial_and_final_positions() {
    // Test that vehicles don't jump to lane edges at start/end of scenario
    let yaml_content = r#"
scenario_type: cut_in_left
time_step: 0.5
duration: 8.0
...
"#;
    ...
    let lane_width = scenario.road.lane_width;

    // Verify initial positions are at lane centers
    for actor in &scenario.actors {
        let initial_state = &actor.states[0];
        if let Some(frenet) = &initial_state.frenet {
            let lane_center = ((actor.states[0].lane() as f64) + 0.5) * lane_width;
            assert!(
                (frenet.t - lane_center).abs() < 0.1,
                "Actor {} should start at lane center ({:.2}m), got {:.2}m",
                actor.id,
                lane_center,
                frenet.t
            );
        ...
        // Verify final position is within road bounds
        let final_state = actor.states.last().unwrap();
        if let Some(frenet) = &final_state.frenet {
            let road_width = scenario.road.num_lanes as f64 * scenario.road.lane_width;
            assert!(
                frenet.t >= 0.0 && frenet.t <= road_width,
                "Actor {} final position {:.2}m should be within road bounds [0, {:.2}]",
                actor.id,
                frenet.t,
                road_width
            );
    ...
    println!("Frenet initial/final position test passed");
    println!("  All vehicles start at lane centers and stay within road bounds");
}
